...
/* ---- the dock's liquid glass lens ------------------------------------- */
// A real lens, not a filter pretending to be one. The shader knows the
// app's own ambient light field (the same bloom the home screen paints)
// and samples it through a thick-glass edge profile: light bends hardest
// at the rim, splits faintly into color, and a specular runs the top edge
// and follows the pointer. The page content underneath still arrives
// through the dock's backdrop blur; this canvas only adds the light.
(function () {
  const canvas = document.getElementById("dockLens");
...
uniform float uDpr;
uniform vec2 uOrigin;   /* dock top-left, css px */
...
vec3 bloomAt(vec2 p) {
  /* The home screen's own twilight, breathing very slowly. */
  vec2 v = uView;
  float w1 = 0.52 + 0.03 * sin(uTime * 0.25);
  float w2 = 0.34 + 0.03 * sin(uTime * 0.21 + 2.1);
  float w3 = 0.16 + 0.02 * sin(uTime * 0.29 + 4.2);
  vec3 c = vec3(0.0);
  vec2 q1 = (p - vec2(0.50, 1.04) * v) / (vec2(1.15, 0.92) * v);
  c += vec3(0.290, 0.275, 0.847) * w1 * pow(clamp(1.0 - dot(q1, q1), 0.0, 1.0), 2.0);
  vec2 q2 = (p - vec2(0.24, 1.08) * v) / (vec2(0.88, 0.76) * v);
  c += vec3(0.486, 0.424, 1.000) * w2 * pow(clamp(1.0 - dot(q2, q2), 0.0, 1.0), 2.0);
  vec2 q3 = (p - vec2(0.78, 1.10) * v) / (vec2(0.84, 0.72) * v);
  c += vec3(0.839, 0.737, 1.000) * w3 * pow(clamp(1.0 - dot(q3, q3), 0.0, 1.0), 2.0);
  if (uLightMode > 0.5) c *= 0.30;
  return c;
}
void main() {
  vec2 uv = vec2(gl_FragCoord.x, uRes.y - gl_FragCoord.y);  /* top-left origin, canvas px */
  vec2 halfR = uRes * 0.5;
  float rad = uRes.y * 0.5;
  vec2 pl = uv - halfR;
  float d = sdRound(pl, halfR, rad);
  if (d > 0.0) { gl_FragColor = vec4(0.0); return; }

  vec2 e = vec2(1.5, 0.0);
  vec2 n = normalize(vec2(
    sdRound(pl + e.xy, halfR, rad) - sdRound(pl - e.xy, halfR, rad),
    sdRound(pl + e.yx, halfR, rad) - sdRound(pl - e.yx, halfR, rad)) + vec2(1e-5));

  /* Lens profile: flat in the middle, bending hard near the rim. */
  float edge = 1.0 - smoothstep(0.0, 16.0 * uDpr, -d);
  float lens = edge * edge;

  vec2 world = uOrigin + uv / uDpr;   /* css px in the viewport */
  float bend = 26.0 * lens;           /* how far the glass reaches sideways */
  vec3 col;
  col.r = bloomAt(world + n * bend * 0.88).r;
  col.g = bloomAt(world + n * bend).g;
  col.b = bloomAt(world + n * bend * 1.14).b;
  col *= 1.0 + 0.35 * lens;           /* the rim gathers light */

  /* A soft interior sheen, brighter toward the top of the pill. */
  float sheen = (1.0 - uv.y / uRes.y) * 0.035;
  col += vec3(sheen);

  /* Specular: a lit band riding the inner rim, strongest where the rim
     faces up, plus a glint that follows the pointer along the top. */
  float rimBand = smoothstep(3.5 * uDpr, 0.8 * uDpr, abs(d + 2.0 * uDpr));
  float topness = clamp(-n.y, 0.0, 1.0);
  float spec = rimBand * pow(topness, 3.0) * 0.55;
  spec += rimBand * pow(clamp(n.y, 0.0, 1.0), 3.0) * 0.10;
  float gx = (uPointer.x - uOrigin.x) * uDpr;
  float glint = exp(-pow(uv.x - gx, 2.0) / (2.0 * pow(46.0 * uDpr, 2.0)));
  spec += rimBand * topness * glint * 0.5;
  if (uLightMode > 0.5) spec *= 0.6;

  vec3 outc = col + vec3(spec);
  float alpha = clamp(max(outc.r, max(outc.g, outc.b)) * 1.5, 0.0, 1.0);
  float aa = smoothstep(0.0, -1.5 * uDpr, d);
  gl_FragColor = vec4(outc * aa, alpha * aa);
}`;
  ...
  const U = {};
  ["uRes", "uDpr", "uOrigin", "uView", "uTime", "uPointer", "uLightMode"].forEach(function (n) {
    U[n] = gl.getUniformLocation(prog, n);
  });

  let px = -1000, py = -1000;
  window.addEventListener("mousemove", function (ev) { px = ev.clientX; py = ev.clientY; }, { passive: true });
  ...
  function frame() {
    ...
    if (dock.offsetParent === null) return;   /* dock hidden: draw nothing */
    const r = size();
    if (!r) return;
    gl.uniform2f(U.uRes, canvas.width, canvas.height);
    gl.uniform1f(U.uDpr, dpr);
    gl.uniform2f(U.uOrigin, r.left, r.top);
    gl.uniform2f(U.uView, window.innerWidth, window.innerHeight);
    gl.uniform1f(U.uTime, (performance.now() - t0) / 1000);
    gl.uniform2f(U.uPointer, px, py);
    gl.uniform1f(U.uLightMode, document.body.classList.contains("light") ? 1 : 0);
