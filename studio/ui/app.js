...
/* The field is gone.
 *
 * Studio used to paint a violet dome with blades, stars and per-pixel
 * grain behind every screen. It was the most technically interesting code
 * in the UI and the wrong thing to show: the app someone is building is
 * the subject, and a lit poster behind it competes for the same attention.
 * Every AI product worth copying reaches the same conclusion -- the chrome
 * recedes so the work reads.
 *
 * Removing it also takes a full-window per-pixel paint and a requestAnimationFrame
 * loop off the machine, which is a real saving on a laptop on battery.
 */
