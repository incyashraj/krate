...
/* ---- the pixel sky ----------------------------------------------------- */
// The website's dot field, tuned down to ambience: twilight pixels
// clustered where the horizon glows, breathing slowly, parting in rings
// around the cursor. Runs at a lazy cadence and skips entirely when the
// window is hidden.
(function () {
  const cv = document.getElementById("pixelSky");
  if (!cv) return;
  ...
  function size() {
    ...
    for (let y = 0; y < rows; y++) for (let x = 0; x < cols; x++) {
      base[y * cols + x] = noise(x * 0.08, y * 0.08) * 0.6 + noise(x * 0.02, y * 0.02) * 0.4;
    }
  }
  size();
  window.addEventListener("resize", size);
  const PAL = ["#221b4e", "#4a46d8", "#7c6cff", "#d6bcff"];
  const ripples = [];
  window.addEventListener("mousemove", (ev) => {
    const r = cv.getBoundingClientRect();
    if (ev.clientY < r.top) return;
    if (ripples.length && performance.now() - ripples[ripples.length - 1].t < 120) return;
    ripples.push({ x: ev.clientX - r.left, y: ev.clientY - r.top, t: performance.now() });
  ...
  function frame(t) {
    ...
    for (let y = 0; y < rows; y++) {
      // Denser toward the bottom, where the horizon light lives.
      const depth = y / rows;
      for (let x = 0; x < cols; x++) {
        let v = base[y * cols + x] * 0.7
              + noise(x * 0.1 + time, y * 0.1 - time * 0.6) * 0.3;
        v *= 0.55 + 0.45 * depth;
        for (let i = 0; i < ripples.length; i++) {
          const rp = ripples[i], age = (t - rp.t) / 1000;
          if (age > 1.3) continue;
        ...
        ctx.globalAlpha = Math.min(1, (v - 0.56) * 2.6);
    ...
    for (let i = ripples.length - 1; i >= 0; i--) {
      if (t - ripples[i].t > 1300) ripples.splice(i, 1);
    }
  }
  requestAnimationFrame(frame);
})();
