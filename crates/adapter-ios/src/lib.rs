//! iOS host adapter surface for Krate Phase 2.
//!
//! This crate is the iOS ownership boundary. Shared behavior still comes from
//! `krate-adapter-common`, while iOS-specific host wiring lands here.
...
impl IosUikitPrototypeUiAdapter {
    ...
    pub fn record_uikit_native_event(
        &self,
        id: WindowId,
    ...
    fn collectors(
        ...
        self.collectors.lock().map_err(|_| {
            UiAdapterError::Internal("ios uikit callback lock is poisoned".to_string())
        })
    }
}

impl WindowAdapter for IosUiAdapter {
...
impl UiAdapter for IosUiAdapter {
...
impl WindowAdapter for IosUikitPrototypeUiAdapter {
    fn info(&self) -> UiAdapterInfo {
    ...
    fn create_window(&self, options: WindowOptions) -> Result<WindowId, UiAdapterError> {
        ...
        let (raw_handle, snapshot) =
            uikit_native::create_native_window(id, &options.title, options.size).map_err(|e| {
    ...
    fn show_window(&self, id: WindowId) -> Result<(), UiAdapterError> {
        uikit_native::show_native_window(id)?;
        WindowAdapter::show_window(&self.headless, id)
    }

    fn close_window(&self, id: WindowId) -> Result<(), UiAdapterError> {
        let _ = uikit_native::close_native_window(id);
        self.remove_session(id);
        WindowAdapter::close_window(&self.headless, id)
    }

    fn set_title(&self, id: WindowId, title: String) -> Result<(), UiAdapterError> {
        if uikit_native::has_native_window(id).unwrap_or(false) {
            uikit_native::set_native_window_title(id, &title)?;
        }
...
impl UiAdapter for IosUikitPrototypeUiAdapter {
    ...
    fn lower_widget_placements(
        ...
        if uikit_native::has_native_window(window).unwrap_or(false) {
            return uikit_native::set_drawn_placements(window, placements);
        }
        Ok(0)
    }
    ...
    fn pump_event_loop_once(
        ...
        if uikit_native::has_native_window(window).unwrap_or(false) {
            for (target, event) in uikit_native::pump_native_events()? {
                self.record_uikit_native_event(target, event)?;
            }
            uikit_native::redraw_all()?;
        }
        Ok(self
...
pub fn discover_ui_adapter() -> IosUiAdapter {
    IosUiAdapter::new()
}
...
mod tests {
    ...
    #[test]
    fn host_family_constant_matches_ios() {
        assert_eq!(HOST_FAMILY, "ios");
    }
    ...
    fn ui_adapter_smoke_creates_blank_draft_window() {
        ...
        assert_eq!(info.backend, "ios-headless-draft");
        ...
        assert_eq!(adapter.backend_name(), "ios-headless-draft");
    ...
    fn winit_prototype_adapter_is_separate_from_default_adapter() {
        ...
        let prototype_adapter = IosUikitPrototypeUiAdapter::new();
        let default_info = default_adapter.info();
        let prototype_info = prototype_adapter.info();

        assert_eq!(default_info.backend, "ios-headless-draft");
        assert_eq!(
        ...
        assert_eq!(prototype_info.backend, "ios-uikit-prototype");
        ...
        assert_eq!(prototype_info.native_windows, cfg!(target_os = "ios"));
        assert_eq!(prototype_info.native_event_loop, cfg!(target_os = "ios"));
        if cfg!(target_os = "ios") {
    ...
    fn winit_prototype_native_window_smoke() {
        ...
        assert!(uikit_native::has_native_window(id).expect("native window tracked"));
        ...
        assert!(!uikit_native::has_native_window(id).unwrap_or(true));
    }
    ...
    fn winit_prototype_collects_callbacks_for_event_loop_pump() {
        ...
        assert!(adapter
            .record_uikit_native_event(id, WinitWindowNativeEvent::Focused(true))
            .expect("record focus"));
        assert!(adapter
            .record_uikit_native_event(id, WinitWindowNativeEvent::Resized(resized))
            .expect("record resize"));
        assert!(adapter
            .record_uikit_native_event(id, WinitWindowNativeEvent::RedrawRequested)
