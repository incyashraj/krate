...
use krate_adapter_common::ui::{
    ...
    UiAdapterError, UiAdapterInfo, UiEvent, UiEventLoopTick, WheelEvent, WidgetId, WidgetNode,
...
fn discover_host_ui_adapter() -> Box<dyn UiAdapter> {
    ...
    #[cfg(target_os = "ios")]
    {
        Box::new(krate_adapter_ios::discover_ui_adapter())
    }
...
fn discover_native_prototype_ui_adapter() -> UiDispatchResult<Box<dyn UiAdapter>> {
    ...
    {
        Ok(Box::new(
            krate_adapter_ios::discover_winit_prototype_ui_adapter()?,
        ))
    }
