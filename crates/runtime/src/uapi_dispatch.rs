...
mod tests {
    ...
    #[test]
    fn net_fetch_rejects_oversized_host_url_before_adapter() {
        let adapter = RecordingAdapter::default();
        let policy =
            SessionPolicy::from_cli_grants(&["net.connect:*:443".to_string()]).expect("policy");
        let guard = UapiGuard::new(policy);
        ...
        let req = HttpRequest {
            method: HttpMethod::Get,
            url: format!("https://{long_host}/v1/ping"),
            headers: Vec::new(),
            body: Vec::new(),
            timeout_millis: None,
        };
        let err = dispatcher
            .net_fetch(req)
            .expect_err("oversized host URL should fail before adapter");
    ...
    #[test]
    fn net_fetch_scheme_case_normalization_matches_grant() {
        let adapter = RecordingAdapter::default();
        let policy =
            SessionPolicy::from_cli_grants(&["net.connect:api.example.com:443".to_string()])
                .expect("policy");
        ...
        dispatcher
            .net_fetch(req)
            .expect("scheme case normalization should match grant");
