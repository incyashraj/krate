    ...
    fn parse(args: impl IntoIterator<Item = String>) -> Result<Self> {
        ...
        while let Some(arg) = args.next() {
            match arg.as_str() {
                ...
                "--help" | "-h" => {
                    println!("Usage: compare-phase2-ucap-evidence --linux <md> --macos <md> --windows <md>");
                    std::process::exit(0);
                }
...
fn parse_step_table(source: &str) -> Result<BTreeMap<String, StepRow>> {
    ...
    if rows.is_empty() {
        bail!("no step rows found in ucap evidence report");
    }
    Ok(rows)
}
...
fn compare_reports(reports: &[HostReport]) -> Result<()> {
    ...
    println!("comparison passed: UCap evidence is aligned");
    Ok(())
}
...
mod tests {
    ...
    fn sample_report(host_os: &str, commit: &str, result: &str, code: i32) -> String {
        format!(
            r#"# Phase 2 UCap Enforcement Evidence

## Host

- Git commit: `{commit}`
- Host: `{host_os}` / `x86_64`
- Generated at (UTC): `2026-05-07T00:00:00Z`
...
"#
    ...
    fn parses_step_rows() {
        ...
        assert_eq!(rows.len(), REQUIRED_STEPS.len());
        assert_eq!(rows[REQUIRED_STEPS[0]].result, "passed");
    }

    #[test]
    fn rejects_failed_step_result() {
        let reports = vec![
        ...
        assert!(err.to_string().contains("Runtime UCap deny matrix"));
    }
