...
fn check_phase3_uapi() -> Result<CheckReport> {
    ...
    let wit_dir = root.join("wit/krate/phase3");
    ...
    let world_id = resolve
        ...
        .context("select krate:app/gui world")?;

    let app_package_name = resolve.packages[app_package].name.to_string();
    ensure(
        app_package_name == "krate:app@0.2.0",
        format!("expected app package `krate:app@0.2.0`, got `{app_package_name}`"),
    )?;
...
fn print_text_report(report: &CheckReport) {
    println!("Krate Phase 3 UAPI draft check passed");
...
fn print_markdown_report(report: &CheckReport) {
    ...
    println!();
    println!(
        "> Generated by `cargo run -p krate-tools --bin check-phase3-uapi -- --format markdown`."
    );
    println!();
