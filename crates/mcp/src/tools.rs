...
impl KrateTools {
    ...
    fn start_build(&self, arguments: &Value) -> Result<Value, String> {
        ...
        // Screen the request before starting a build that cannot succeed.
        //
        // This matters more here than at the command line. Someone who typed
        // `krate create` knows what they asked for and reads the output. A
        // model that gets a job id will poll it, see a green check, and tell
        // the person their email app is ready -- because every mechanical
        // stage passed. The app builds, imports zero OS calls, runs, and
        // paints a frame; none of that is about whether it can do what was
        // asked. Refuse in one second with the reason and the nearest thing
        // Krate can actually build.
        if let krate_author::feasibility::Verdict::Refuse(refusal) =
            krate_author::feasibility::screen(description)
        {
            return Err(format!(
                "Krate cannot build that. {}\n\nAsk for this instead: {}\n\n\
                 Tell the person plainly rather than trying a different wording \
                 of the same request.",
    ...
    fn build_status(&self, arguments: &Value) -> Result<Value, String> {
        ...
        match job.phase {
            ...
            Phase::Succeeded => {
                result["output"] =
                    json!(job.output.as_ref().map(|path| path.display().to_string()));
    ...
    fn check(&self, arguments: &Value) -> Result<Value, String> {
        ...
        let (dir, _scratch) = if let Some(dir) = arguments.get("dir").and_then(Value::as_str) {
            ...
            return Err(
                "krate_check needs one of `files` (the app source as a map of relative \
    ...
    fn package(&self, arguments: &Value) -> Result<Value, String> {
        let id = arguments
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| {
                "krate_package needs `job_id`, the id of a build that succeeded".to_string()
            })?;
        let job = self.jobs.get(id).ok_or_else(|| self.no_such_job(id))?;
    ...
    fn materialize(&self, files: &Value) -> Result<(PathBuf, PathBuf), String> {
        ...
        if map.is_empty() {
            return Err(
                "`files` is empty; pass at least Cargo.toml, src/lib.rs, and \
                        manifest.toml."
                    .to_string(),
            );
        }
...
mod tests {
    ...
    fn tools(root: &Path) -> KrateTools {
        KrateTools::new(root.join("no-such-krate"), root.to_path_buf(), |_| {
            "PACK".to_string()
        })
    }
