...
pub fn handle_line(tools: &dyn ToolSet, raw: &str) -> Option<Value> {
    ...
    let Some(method) = message.get("method").and_then(Value::as_str) else {
        ...
        return id
            .map(|id| error_response(id, INVALID_REQUEST, "message has no `method` field", None));
    };
    ...
    let result = match method.as_str() {
        "initialize" => match initialize(tools, &params) {
            Ok(result) => result,
            Err(err) => return Some(error_response(id, INVALID_PARAMS, &err.message, err.data)),
        },
        "ping" => json!({}),
...
mod tests {
    ...
    fn an_unknown_tool_is_a_protocol_error_that_names_the_real_ones() {
        let response =
            call(r#"{"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"nope"}}"#);
    ...
    fn a_tool_with_no_arguments_key_is_called_with_an_empty_object() {
        ...
        let response =
            call(r#"{"jsonrpc":"2.0","id":8,"method":"tools/call","params":{"name":"echo"}}"#);
        assert!(response.get("error").is_none());
        assert_eq!(response["result"]["isError"], true);
    }
    ...
    fn batches_are_refused_rather_than_silently_dropped() {
        ...
        let response =
            handle_line(&Fake, r#"[{"jsonrpc":"2.0","id":1,"method":"ping"}]"#).expect("response");
