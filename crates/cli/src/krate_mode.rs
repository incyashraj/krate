...
mod tests {
    ...
    fn the_worked_examples_are_real_shipped_source() {
        ...
        assert!(
            ...
            "the checklist's and the clock's real export line"
        );
        // One shape, everywhere (K-338): the per-app bindings wiring the
        // chat pack used to teach beside the SDK path is gone.
        for old in [
            ...
            assert!(
                !prompt.contains(old),
                "the chat pack still teaches the per-app bindings wiring: {old:?}"
            );
        }
        assert!(
