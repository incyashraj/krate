...
impl Client {
    ...
    fn notify(&mut self, method: &str) {
        ...
        writeln!(self.stdin, "{}", serde_json::to_string(&message).expect("encode"))
    ...
    fn call_tool_expecting_error(&mut self, name: &str, arguments: Value) -> String {
        let response = self.request("tools/call", json!({ "name": name, "arguments": arguments }));
        assert!(response.get("error").is_none(), "{name} must not be a protocol error");
        assert_eq!(response["result"]["isError"], true, "{name} should have failed");
...
fn a_client_can_initialize_list_tools_and_call_the_cheap_ones() {
    ...
    for example in list {
        ...
        assert!(lib.len() > 1000, "{} is too short to be a whole app", example["name"]);
...
fn a_client_can_drive_a_build_from_start_to_a_finished_krate_file() {
    ...
    assert_eq!(
        package["structuredContent"]["bytes"].as_u64().expect("bytes"),
        on_disk,
...
fn the_oracle_reports_a_broken_app_with_the_stage_and_the_fix() {
    ...
    assert_eq!(stage, "layout", "check-app should fail at the first missing thing");
    assert!(
        verdict["fix"].as_str().expect("a fix").contains("manifest.toml"),
