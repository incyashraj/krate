//! Windows host adapter surface for Krate Phase 2.
//!
//! This crate is the Windows ownership boundary. Shared behavior still comes
//! from `krate-adapter-common`, while Windows-specific host wiring will land
//! here.
...
impl WindowsUiAdapter {
    ...
    /// Attach a future winit or Win32-owned window handle to a Krate window id.
    ///
...
impl WindowsWinitPrototypeUiAdapter {
    ...
    ///
    /// True in Windows builds: the winit backend is compiled in.
    pub fn native_windows_enabled(&self) -> bool {
    ...
    fn collectors(
        ...
        self.collectors.lock().map_err(|_| {
            UiAdapterError::Internal("windows winit callback lock is poisoned".to_string())
        })
    }
}
...
impl WindowAdapter for WindowsWinitPrototypeUiAdapter {
    ...
    /// Full-bleed on Windows is an undecorated window (K-117, K-148).
    ///
    /// The pack encourages full-bleed designs and macOS has shipped them since
    /// K-117, but Windows had no implementation at all, so the trait default
    /// refused and every such app got standard chrome instead -- silently,
    /// because the app ignores the error and carries on.
    ///
    /// Losing the title bar changes the client area, so the new size is queued
    /// as a resize; without that the canvas keeps its old dimensions and leaves
    /// a stale band where the frame used to be.
    fn set_full_bleed(&self, id: WindowId, enabled: bool) -> Result<(), UiAdapterError> {
        if winit_native::has_native_window(id).unwrap_or(false) {
...
mod tests {
    ...
    fn winit_prototype_adapter_is_separate_from_default_adapter() {
        ...
        assert_eq!(prototype_info.native_windows, cfg!(target_os = "windows"));
        assert_eq!(
    ...
    fn winit_prototype_tracks_session_and_pumps_step() {
        ...
        // is covered by the ignored native smoke).
        let id = adapter
            .headless
    ...
    /// Real winit window round trip. Run on a Windows host with
    /// `KRATE_WINIT_NATIVE_SMOKE=1 KRATE_WINIT_ANY_THREAD=1
    ///  cargo test -p krate-adapter-windows -- --ignored`.
    #[test]
    #[ignore = "needs a Windows desktop session; opt-in native smoke"]
