...
> **Scroll v0 map (scoped 2026-07-20 — DONE same day, certification pending the next full matrix):** host-side scrolling, zero WIT changes — the guest declares a `Scroll` node with children and stays oblivious; offset, clipping, and wheel input are runtime concerns (matching native platform feel). (1) Layout: widgets with an explicit height get `flex_shrink: 0` so Scroll children overflow instead of compressing (Taffy default shrink=1 was silently squeezing). (2) `WidgetPlacement` gains `clip: option<rect>`; both painters intersect fills with the clip; labels render only when their row fits inside it (partial-row text is deferred to the vello clip-layer pass). (3) Winit hosts capture `MouseWheel` into a `RawWheelSample` drain channel (the pointer/key pattern). (4) The gui host keeps `scroll_offsets` per (window, scroll-widget); placements under a Scroll subtract the offset and carry the container's rect as clip; wheel samples hit-test the topmost Scroll under the cursor, clamp offset to content extent, and re-lower. (5) hello-gui grows a scroll area with eight text rows below the existing widgets (CI click coordinates untouched); the Xvfb proof adds wheel clicks (xdotool button 5) before the screenshot so the artifact shows scrolled content. (6) Definition of done: unit tests pin offset clamping and clip math; the screenshot shows a mid-list scroll position.
>
> **Widget state map (scoped and DONE 2026-07-06):** `widget-node` gained `checked: option<bool>` + `value: option<f32>` in the draft WIT (pre-freeze window, exactly what A4 reserved it for); host validates value in 0..=1; placements carry state; both painters render Checkbox/Radio/Switch/Slider/Progress; AppKit `kind_supported` still gates native lowering to Button/TextField/Text (native stateful controls are a later slice); hello-gui demos an interactive checkbox + typing-driven progress bar. Remaining in this lane: state-change semantics for native lowering, Scroll containers + wheel events (WIT), ListView, then the notes flagship.
>
> **Keyboard input map (scoped and DONE 2026-07-05, host side):** winit `KeyboardInput`/`ModifiersChanged` → `RawKeySample` drain channel (mirror of the pointer pattern; raw samples never enter the event queue) → gui host attaches `focused_widget` and queues portable `key`/`text-input` events; pointer presses routed onto `TextField`/`TextArea` call `focus_node` (click-to-focus with portable `focus-changed`). Zero WIT changes. Covered by dispatcher round-trip tests and a focusable-kinds unit test. The visible typing round trip landed the same day: hello-gui renders text-input events live (fixed-capacity panic-free buffer — indexed slice ops would pull WASI panic machinery), Backspace works, and the Xvfb proof types "hi krate" via xdotool, screenshots the window with the text visible, and asserts the guest echoes `typed:hi krate`. Remaining for the input lane: **keyboard input on macOS entirely** — `adapter-macos` has no `RawKeySample` path at all and inherits the empty default `drain_raw_key_input`, so an AppKit window delivers pointer events but no keys and `Modifiers::default()` (all false) everywhere, which means no typing and no Cmd+S. Linux and Windows both populate keys and modifiers from winit. Then key repeat policy, macOS native-field text sync back to the guest, and IME (§7.2 IBus / composition events) later. Verified 2026-07-23 while testing krate-notes: the flagship is clickable but not typable on macOS. UPDATE 2026-07-23: keyboard typing, deletion, and retyping now round-trip on macOS via a new text-changed (replace-semantics) event; the guest mirrors the native control rather than owning the text. Still missing on macOS: **clipboard paste (Cmd+V)**, because the bare AppKit window has no menu bar, and macOS wires the standard Cut/Copy/Paste shortcuts through the application Edit menu to the first responder. The fix is an NSMenu with the standard edit items; the text field already implements paste:/copy:, so the shortcuts connect automatically once the menu exists. Tracked as the next macOS slice.
>
> **Renderer slice map (scoped 2026-07-04 — first pass DONE 2026-07-05: shared painter extracted to `adapter-common/src/painter.rs`, then `vello_cpu` 0.0.9 + `parley` 0.11 render the whole frame with antialiased system-font labels behind the `vector-text` feature; bitmap painter kept as the zero-dependency fallback; antialiasing asserted by a shade-counting unit test. Styling landed later the same day: rounded corners, hover/pressed button fills via `PaintInteraction` + shared hit-testing in both winit hosts. Remaining from this map: the eventual GPU vello swap of the same painter, and broader theme tokens per §11.5 as widgets grow):** replace flat-color drawing with real 2D rendering behind the *same* placement contract — `draw_placements` keeps its signature; only the painter changes. (1) Evaluate `vello_cpu` (the CPU sparse-strip rasterizer from the vello project) + `parley` (text layout) rendering into the CPU pixel buffer that softbuffer already presents — this keeps CI free of GPU requirements (hosted runners have no usable Vulkan) while producing antialiased vector widgets and real typography; GPU vello lands later as a swap of the same painter. (2) Widgets gain rounded corners, borders, hover/pressed fills from per-host theme tokens (§11.5). (3) The 5x7 bitmap `drawtext` module stays as the zero-dependency fallback and test baseline. (4) Verification: existing unit-shape tests must not change (placement contract untouched); the Xvfb click proof and screenshot artifact prove the new painter visually; a pixel-sample test asserts text pixels are antialiased (multiple alpha levels), not bilevel. (5) Definition of done: hello-gui's Linux screenshot artifact shows antialiased labels and the synthetic click proof stays green.
>
> **Drawn text v0 (DONE 2026-07-04):** 5x7 bitmap font module `adapter-common/src/drawtext.rs` (95 printable-ASCII glyphs, integer scaling, per-pixel clipping) wired into `draw_placements` on Linux and Windows: button labels centered, field text left-inset, `Text` widgets render glyphs instead of blocks. Explicit placeholder until the renderer slice above.
>
> **Winit input-routing map (scoped 2026-07-04 — DONE same day, certified by full matrix run `28708285696`: xdotool click under Xvfb pressed the drawn button and hello-gui exited 0):** make the drawn widgets interactive on Linux and Windows. (1) In both `winit_native` modules, handle `WindowEvent::CursorMoved` (track the last logical cursor position per window, dividing physical by `scale_factor`) and `WindowEvent::MouseInput` (left button press/release). (2) Extend the shared `WinitWindowNativeEvent` in `adapter-common` with a `Pointer { x, y, pressed }` variant; the collector, session apply path, and pump already forward variants — the new arm queues `queue_pointer_event` with `widget: None` (adapters do not hit-test; layout lives runtime-side). (3) In `Phase3GuiHost::poll_one_event`, intercept `UiEvent::Pointer` events whose `widget` is `None`: re-route them through `dispatcher.route_pointer_event` with the window's record size as the viewport so hit testing assigns the widget id, then deliver the routed event and drop the raw one. hello-gui already reacts to `pointer.widget == Some(BUTTON_ID)`, so no guest change. (4) CI proof: in the Linux Xvfb step, after launching hello-gui with `--native-window` in the background, use `xdotool` (apt-install alongside xvfb, run under the same `xvfb-run` server via a wrapper script) to click the button's known coordinates (placements are deterministic: root padding 16 logical, button 160x32 at the top of the stack), then assert exit code 0 — the first fully machine-verified click round trip on Linux. Windows lane keeps exit-1 (no synthetic input tooling there yet). (5) Definition of done: hello-gui exits 0 under Xvfb from a synthetic click on the drawn button.

> **Winit slice map (scoped 2026-07-03 — DONE 2026-07-03/04: real winit windows on Linux and Windows in the full matrix, plus drawn-widget rectangles):** real Linux/Windows winit windows behind the existing prototype boundaries. (1) Add `winit 0.30` as a target-gated dependency of `adapter-linux` (features `x11`,`wayland`) and `adapter-windows`; macOS builds stay untouched. (2) Own the `EventLoop` thread-locally per process (mirror `APPKIT_PROTOTYPE_SESSIONS` in `adapter-macos`), created with `EventLoop::new()` + `EventLoopExtPumpEvents::pump_app_events(Some(Duration::ZERO), ..)` for the non-blocking tick — winit 0.30 drives an `ApplicationHandler`; implement it to push `WindowEvent`s (CloseRequested, Resized, Focused, ScaleFactorChanged, RedrawRequested) into the existing `WinitWindowEventCollector` as `WinitWindowNativeEvent`s, which the shared pump already drains. (3) Window creation via `event_loop.create_window(WindowAttributes)` inside the first pump; attach the handle through the existing `attach_winit_session` path. (4) Linux widgets: drawn fallback only per ADR-0015 — window + events first, drawing later; Windows native Win32 lowering comes after windows exist. (5) Verification loop: unit-shape tests headless; fast CI's ubuntu lane compiles the Linux path on every push; the full matrix compiles Windows; real window smokes run under `xvfb-run` on ubuntu (install in the full-test lane) and natively on the windows runner, both behind an env-gated ignored test like the AppKit smoke. (6) Definition of done for the slice: `krate run --native-window` opens a real window for hello-gui on a Linux host with events flowing (drawn widgets may still be absent — window + close/resize/focus round trip is the slice).
...
trait, an explicit `WindowAdapter` trait, host-crate UI adapter entry points,
runtime host-adapter discovery, a runtime-facing UI dispatcher, the first shared widget tree model, draft
...
primitive. Draft host window events for close request, resize, focus, theme,
...
frame snapshot for local smoke proof. The real AppKit window delegate object
now exists too: it implements `NSWindowDelegate`, records close, resize, focus,
and backing-scale callbacks, and lets `AppKitWindowSession` drain them through
the same tested Rust bridge.
...
- ADR-0013 is now drafted. It still needs review before it becomes accepted.
...
- 2026-05-21: Added the prepared layout path. Reusing the same Taffy tree brings
...
- 2026-05-22: Split the named `WindowAdapter` boundary from the larger
  `UiAdapter` path. Window lifecycle and host-window events are now the lower
...
- 2026-06-07: Added AppKit draw-surface state. It tracks the window size,
...
- 2026-06-20: Added the first real AppKit window delegate object. It implements
  `NSWindowDelegate`, records native close, resize, focus, and backing-scale
...
- 2026-06-20: Added the first AppKit event-loop step driver. It can process one
  ...
  headless CI.
- 2026-06-20: Added selectable AppKit prototype runtime mode. The normal
  `Phase3UiRuntime::with_host_adapter` path still uses the headless adapter,
  while `Phase3HostUiMode::NativePrototype` can select the macOS AppKit
  prototype adapter and report native window plus native event-loop support.
- 2026-06-20: Added the shared event-loop pump boundary. `UiAdapter` and
...
- 2026-06-21: Added Linux and Windows Winit prototype boundaries. Both host
  adapter crates now have explicit prototype adapter types, native-handle
