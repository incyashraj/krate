...
| 2 | UAPI v0.1 (CLI) | Started | 2026-05-03 | pending | Draft WIT package, manifest/policy checks, manifest entry/run-file match check, terminal grant prompt, effective capability dump, runtime guard, host-binding checkpoint, dispatcher scaffold, generated type bridge, generated host wiring, resource table, initial Phase 2 `layer36 run` linker path, smoke happy/denied proofs, first Rust SDK crate and helper layer, `layer36-clock`, first `layer36-cat`, first `layer36-curl`, permission-denied exit-code alignment, Rust SDK docs, and budget-aware CI mode are in place. |
