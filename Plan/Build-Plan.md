...
> **Current Phase:** Phase 3 desktop UI foundation is underway, while Phase 2 waits for outside review before formal closeout
...
| 2026-05-16 | Opted GitHub Actions workflows into the Node 24 JavaScript action runtime and moved cache/artifact/Pages upload steps to Node 24 action majors where available. |
| 2026-05-17 | Added a timed walkthrough evidence checker so the external Rust walkthrough packet must include filled metadata, numeric timing, a pass/fail result, and step results before `P2E-12` can be accepted. |
| 2026-05-17 | Added the Phase 2 retrospective draft, Phase 3 kickoff issue draft, and a closeout-docs checker so handoff material exists without claiming Phase 2 is complete too early. |
| 2026-05-17 | Wired the Phase 2 closeout-docs checker into hosted UAPI CI, self-hosted full gate, and the exit bundle so handoff docs are checked with the rest of the Phase 2 exit proof. |
...
| 2026-05-23 | Added the AppKit delegate callback bridge. AppKit-style callback names now pass through one tested Rust translator before reaching the shared event state. |
| 2026-06-07 | Added AppKit draw-surface state. The macOS adapter now tracks surface size, scale, clear color, redraw count, and frame metadata before the real `NSView` painter lands. |
| 2026-06-20 | Added AppKit draw view surface. The opt-in macOS prototype can now attach an owned `NSView`, set a visible clear color, mark it dirty, and record a first frame snapshot. |
| 2026-06-20 | Added AppKit native window delegate object. The opt-in macOS prototype can now retain an `NSWindowDelegate`, collect close, resize, focus, and backing-scale callbacks, and drain them through the existing Rust event bridge. |
| 2026-06-20 | Added AppKit event-loop step driver. The opt-in macOS prototype can now process one native tick by refreshing state, draining delegate callbacks, and queueing redraw through the shared event stream. |
| 2026-06-20 | Added selectable AppKit prototype runtime mode. The default runtime remains headless, but macOS can now explicitly request the AppKit prototype adapter and report native event-loop capability. |
| 2026-06-20 | Added shared event-loop pump boundary. The runtime can now ask any UI adapter for one non-blocking event-loop tick; headless adapters return no native tick and AppKit maps its prototype tick into the shared report. |
| 2026-06-21 | Added selectable AppKit runtime smoke command. The local macOS proof now creates, shows, pumps, inspects, and closes the native prototype path through `Phase3UiRuntime` and `Phase3UiDispatcher`. |
