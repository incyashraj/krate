...
| P2-TEST-06 | Phase 2 cross-host sample evidence comparator | 2026-05-06 | Added `layer36-tools --bin compare-phase2-sample-evidence` and `scripts/compare-phase2-sample-evidence.sh`. The comparator reads Linux/macOS/Windows sample-evidence reports, verifies required sample statuses, and fails if clock/cat/curl stdout hashes drift across hosts. It also supports a temporary `--allow-blocked-curl` mode for restricted localhost environments while still enforcing clock/cat parity. |
