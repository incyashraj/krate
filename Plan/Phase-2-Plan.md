...
| P2-CI-07 | Hosted full CI TypeScript fixture default gate | 2026-05-05 | Updated hosted full-test CI to run language-variant fixtures in `ts` mode by default and enabled `npx` jco installation in the fixture build step. This keeps the TypeScript runtime fixture lane active by default in full hosted CI while preserving manual mode override inputs. |
