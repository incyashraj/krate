...
| 4 | Go (TinyGo) bindings generated and usable; sample builds and runs | Advanced: `layer36 doctor` now reports TinyGo and Go readiness, `packages/sdk-go` has the helper scaffold and examples, and self-hosted CI now runs a TinyGo WASI Preview 2 build-smoke lane for clock/cat/curl with component-shape verification plus a runtime-fixture promotion gate that only activates when imports are Layer36-pure; generated binding module and full Layer36-runtime fixture proof remain |
...
| Formal Phase 2 exit | About 69-74% complete | Language runtime proofs, seven-day CI evidence, fuzzing, full benchmark gate, and external validation. |
...
| P2-TEST-04E | Go curl non-localhost error-path assertions | 2026-05-05 | Added Go curl fixture assertions for missing-grant denial and unresolved-host failure paths that do not require localhost fixture sockets. Updated Go curl sample stderr format to include explicit `layer36-go-curl` error lines and a stable `fetch failed` marker. |
