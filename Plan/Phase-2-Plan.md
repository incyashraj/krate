...
| P2-CI-41 | Windows sample evidence CLI path | 2026-05-18 | Hosted full CI run `26050285568` proved Linux and macOS full-test lanes green, then found that Windows sample evidence looked for `target/debug/layer36` instead of Cargo's Windows binary `target/debug/layer36.exe`. The recorder now resolves the built CLI binary with an optional `LAYER36_BIN` override and a Windows `.exe` fallback before running sample evidence. |
