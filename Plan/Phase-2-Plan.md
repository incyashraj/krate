...
| P2-BIND-02G | Shared language-fixture builder Go promotion integration | 2026-05-06 | `scripts/build-phase2-language-variant-fixtures.sh` now auto-attempts Go runtime fixture promotion through `scripts/promote-phase2-go-runtime-fixtures.sh` when `go`, `tinygo`, and `wasm-tools` are available. This unifies TypeScript and Go fixture activation behind one build step while still keeping Go fixtures disabled when Layer36 import-purity checks do not pass. |
