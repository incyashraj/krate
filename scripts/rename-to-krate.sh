#!/bin/bash
# Amendment A9 executor: rename Krate -> Krate across the repository.
#
...
#        `krate-local` to `krate-local`, but the physical runner keeps its
...
#
# The schema id krate.run.v1 replaces krate.run.v1; keep the old id
...
# ---- 1. Content replacement: krate->krate, Krate->Krate, KRATE->KRATE,
...
COUNT=0
for f in $FILES; do
  if grep -qi "krate" "$f"; then
    COUNT=$((COUNT+1))
    if [ "$APPLY" = "1" ]; then
      sed -i '' -e 's/krate/krate/g' -e 's/Krate/Krate/g' -e 's/KRATE/KRATE/g' "$f" 2>/dev/null \
        || sed -i -e 's/krate/krate/g' -e 's/Krate/Krate/g' -e 's/KRATE/KRATE/g' "$f"
    fi
  fi
done
...
PATHS=$(git ls-files | grep -i "krate" | awk -F/ '{ for(i=1;i<=NF;i++){ if (tolower($i) ~ /krate/) { out=""; for(j=1;j<=i;j++) out=out (j>1?"/":"") $j; print out; break } } }' | sort -u)
echo "paths containing krate:"
echo "$PATHS" | sed 's/^/  /'
if [ "$APPLY" = "1" ]; then
  # Deepest paths first so parents rename cleanly afterward.
  echo "$PATHS" | awk '{ print gsub(/\//,"/"), $0 }' | sort -rn | cut -d' ' -f2- | while read -r p; do
    [ -e "$p" ] || continue
    np=$(echo "$p" | sed -e 's/krate/krate/g' -e 's/Krate/Krate/g')
    [ "$p" = "$np" ] && continue
    git mv "$p" "$np"
    echo "  moved: $p -> $np"
  done
